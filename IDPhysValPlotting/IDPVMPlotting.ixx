...
template <class H>
void IDPVMPlotting::draw1DWithRatio(PlotContent<H> &pc)
{
    SetAtlasStyle();
    ...
    if (pc.getMultiPagePdfHandle())
        pc.getMultiPagePdfHandle()->saveCanvas(mpc.getCanvas());
}

template <class H>
void IDPVMPlotting::draw1DNoRatio(PlotContent<H> &pc)
{
    SetAtlasStyle();
    // first, fill the histos we want
    pc.populateAll();
    ...
    if (pc.getMultiPagePdfHandle())
        pc.getMultiPagePdfHandle()->saveCanvas(can);
}

template <class Histo>
void IDPVMPlotting::draw2DPlots(PlotContent<Histo> &pc)
{
    SetAtlasStyle();
    pc.populateAll();
    CanvasOptions canvasOpts = pc.getCanvasOptions();
    ...
    for (auto &plot : plots)
    {
        ...
        if (fmt.ExtraDrawOpts.isUserSet())
            theOpts = fmt.ExtraDrawOpts();
...
template <class H>
void IDPVMPlotting::draw1DNoRatio(std::vector<PlotContent<H>> &pcs)
{
    for (auto &pc : pcs)
        draw1DNoRatio(pc);
}
template <class H>
void IDPVMPlotting::draw1DWithRatio(std::vector<PlotContent<H>> &pcs)
{
    for (auto &pc : pcs)
        draw1DWithRatio(pc);
}
template <class H>
void IDPVMPlotting::draw1D(std::vector<PlotContent<H>> &pcs)
{
    for (auto &pc : pcs)
        draw1D(pc);
}
template <class H>
void IDPVMPlotting::draw2DPlots(std::vector<PlotContent<H>> &pcs)
{
    for (auto &pc : pcs)
        draw2DPlots(pc);
}
...
/// ... I'm not judging... !
template <class P, typename... other>
void IDPVMPlotting::draw1DNoRatio(P &pcs, other... others)
{
    draw1DNoRatio(pcs);
    draw1DNoRatio(others...);
}
template <class P, typename... other>
void IDPVMPlotting::draw1DWithRatio(P &pcs, other... others)
{
    draw1DWithRatio(pcs);
    draw1DWithRatio(others...);
}
template <class P, typename... other>
void IDPVMPlotting::draw1D(P &pcs, other... others)
{
    draw1D(pcs);
    draw1D(others...);
}
template <class P, typename... other>
void IDPVMPlotting::draw2DPlots(P &pcs, other... others)
{
    draw2DPlots(pcs);
    draw2DPlots(others...);
}
